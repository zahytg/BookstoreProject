...
export default function AdminCategories({ token }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.msg || 'Lỗi xử lý danh mục. Hãy kiểm tra lại Backend.')
    }
  }
  ...
  return (
    <div className="container py-5">
      <h2 className="text-center mb-5 fw-bold" style={{ color: themeColor }}>Quản lý danh mục</h2>
      ...
      <div className="card shadow mb-5" style={{ borderRadius: '15px', border: 'none' }}>
        <div className="card-body p-4 p-md-5">
          <div className="d-flex justify-content-between align-items-center mb-4">
            ...
            <Link to="/admin/books" className="btn btn-sm btn-outline-secondary px-3 fw-bold shadow-sm" style={{ borderRadius: '8px' }}>
              ← Quản lý sách
            </Link>
          </div>
        ...
        <table className="table table-hover table-bordered mb-0 align-middle">
          ...
          <tbody>
            {currentItems.length > 0 ? currentItems.map(cat => (
              ...
              <tr><td colSpan="2" className="text-center py-4 text-muted">Chưa có dữ liệu hiển thị.</td></tr>
            )}
          </tbody>
        </table>
      </div>
              ...
              <li key={i} className={`page-item mx-1 ${currentPage === i + 1 ? 'active' : ''}`}>
                <button 
                  ...
                  style={currentPage === i + 1 ? 
                    { backgroundColor: themeColor, borderColor: themeColor, color: 'white' } : 
