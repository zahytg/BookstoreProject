...
export default function Checkout({ token }) {
  ...
  const [createdOrder, setCreatedOrder] = useState(null) // ✅ Lưu đơn hàng vừa tạo
  ...
  // ✅ Chỉ còn 2 vouchers (đã xóa FREESHIP)
  const validVouchers = {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (res.status === 201 || res.data) {
        ...
        setCreatedOrder(res.data.order) // ✅ Lưu đơn hàng vừa tạo
        localStorage.removeItem('cart')
        // ✅ XÓA AUTO REDIRECT
      }
    } catch (err) {
      console.error('Lỗi đặt hàng:', err)
      alert('Đặt hàng thất bại: ' + (err.response?.data?.msg || 'Lỗi server'))
    } finally {
      setLoading(false)
    }
  }

  // ✅ TRANG THÀNH CÔNG VỚI 2 NÚT
            ...
            <div className="card-body p-4">
              ...
              <div className="mb-4">
                ...
                <div className="form-check">
                  ...
                  />
                  <label className="form-check-label">
                    <i className="bi bi-credit-card me-1"></i>Thanh toán online
                  </label>
                </div>
              </div>
